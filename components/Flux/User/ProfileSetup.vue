  ...
  <div>
    ...
    <form @submit.prevent="onCreateFluxProfile">
      <div class="mb-4">
        ...
        <p class="text-gray-600 text-sm mt-1 pl-4">
          Only letters, numbers, hyphens and underscores allowed
        </p>
      </div>
      <div class="mb-4">
        <label for="displayName" class="block mb-2 font-bold"
          >Display Name</label
        >
        <UInput
          v-model="displayName"
        ...
        <p class="text-gray-600 text-sm mt-1 pl-4">
          Anything you want, up to 25 characters.
        </p>
      </div>
      <button type="submit" class="bg-nuclear-400 text-white px-4 py-2 rounded">
        Create Profile
      </button>
    </form>
  </div>
...
const onCreateFluxProfile = async () => {
  ...
  try {
    const profile = await fluxService.createMyFluxProfile(
      handle.value,
      displayName.value
    );
